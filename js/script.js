...
const knowledgeBase = {
  ...
  puedeTomar: function (curso, cursosAprobados) {
    ...
    const faltantes = requisitosArray.filter(req => !cursosAprobados.includes(req));
...
class Chatbot {
  ...
  procesarPregunta(pregunta) {
    ...
    if (
      ...
      if (match) {
        ...
        const contieneOperador = operadores.some(op => expresion.toUpperCase().includes(op));

        if (!contieneOperador) {
          return `Operador no válido. Solo se permiten los operadores <strong>AND</strong>, <strong>OR</strong> y <strong>NOT</strong>.`;
        }

        const resultado = this.evaluarExpresionLogica(expresion, this.cursosAprobados);
        return resultado
      ...
      const cursoMatch = pregunta.match(/necesito saber si puedo tomar\s+(.+?)\s+habiendo/i);
      ...
      if (match && match[1]) {
        ...
        const cursosDisponibles = Object.keys(knowledgeBase.cursos)
          .filter(curso => {
            const resultado = knowledgeBase.puedeTomar(curso, cursosSimulados);
      ...
      if (match && match[1] && match[3]) {
        ...
        const cursosSimulados = [...this.cursosAprobados.map(c => c.toLowerCase()), cursoAprobado.toLowerCase()];

        const resultado = knowledgeBase.puedeTomar(cursoDeseado, cursosSimulados);
    ...
    else if (preguntaLower.includes('qué días se imparte') || preguntaLower.includes('que dias se imparte')) {
      const curso = this.buscarCurso(pregunta);
      if (!curso) {
        const cursosDisponibles = Object.keys(knowledgeBase.cursos).join(', ');
        return `No reconozco ese curso. Cursos disponibles: <div class="course-list">${cursosDisponibles}</div>`;
    ...
    else if (preguntaLower.includes('cuántos créditos vale') || preguntaLower.includes('cuantos creditos vale')) {
      const curso = this.buscarCurso(pregunta);
      if (!curso) {
        const cursosDisponibles = Object.keys(knowledgeBase.cursos).join(', ');
        return `No reconozco ese curso. Cursos disponibles: <div class="course-list">${cursosDisponibles}</div>`;
    ...
    // Consulta sobre requisitos para tomar un curso
      ...
      if (preguntaLower.includes('sin haber aprobado')) {
        ...
        if (match && match[1]) {
          ...
          if (this.cursosAprobados.map(c => c.toLowerCase()).includes(cursoNoAprobado.toLowerCase())) {
  ...
  evaluarExpresionLogica(expresion, cursosAprobados) {
    ...
    const cursosAprobadosLimpios = cursosAprobados.map(curso => curso.trim().toUpperCase());
    ...
    if (expresionLimpia.includes('OR')) {
      ...
      resultado = partes.some(parte => this.evaluarParte(parte.trim(), cursosAprobadosLimpios));
    }
    ...
    // Evaluar AND por defecto
      ...
      resultado = partes.every(parte => this.evaluarParte(parte.trim(), cursosAprobadosLimpios));
    }
    ...
    // Si la expresión contiene 'NOT', solo se evalúa cuando sea necesario
      ...
      resultado = partes.every(parte => !cursosAprobadosLimpios.includes(parte.trim()));
    }
